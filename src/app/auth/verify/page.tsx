...
export default function VerifyPage() {
  ...
  useEffect(() => {
    if (token) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- Async data fetching with state update is intentional
      verifyEmail();
    }
  }, [token, verifyEmail]);

  // Handle no token case outside of useEffect
  if (!token) {
    return (
        ...
        <div className="w-full max-w-md text-center">
          ...
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8">
            ...
            <div className="space-y-3">
              ...
              <Link
                href="/auth/login"
                className="block w-full py-3 px-4 bg-white/5 hover:bg-white/10 text-white font-semibold rounded-lg transition-colors border border-white/10"
              >
                {t('backToLogin')}
              </Link>
            </div>
